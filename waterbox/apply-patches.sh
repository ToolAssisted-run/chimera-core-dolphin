#!/bin/sh
# Applies the numbered patches to the extern/dolphin submodule. Idempotent: a tree
...
root="$(cd "$here/.." && pwd)"
# DOLPHIN_TREE points it at another checkout: how this script is tested without
# touching the tree a build depends on
tree="${DOLPHIN_TREE:-$root/extern/dolphin}"

if [ "$(git -C "$tree" rev-parse --show-toplevel 2>/dev/null)" != "$(cd "$tree" 2>/dev/null && pwd -P)" ]; then
	echo "extern/dolphin is not checked out; run:" >&2
	echo "  git -C $root submodule update --init --recursive extern/dolphin" >&2
	exit 1
fi
...
if [ "$wrong" -ne 0 ]; then
	echo "extern/dolphin is partly patched. To start again from the submodule's HEAD:" >&2
	echo "  git -C extern/dolphin reset --hard && git -C extern/dolphin clean -fd && waterbox/apply-patches.sh" >&2
