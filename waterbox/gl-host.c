 ...
 * video.rebuildOnStateLoad false. Dolphin declares nothing, so it rebuilds. */
 ...
 * did that for. run-wbx and run-native say this out loud at the end of a run and the
                                              ...
                                              uintptr_t c, uintptr_t d, uintptr_t e)
{
	...
	switch (op)
	{
		...
		case GL_OP_CONTEXT_ID:
			 ...
			 * (chimera_dolphin_gl_frame_start, OGLGfx.cpp) and rebuilds them
			 * when it moves. Answering 0, which is what this host did before
			 * it had this case, means "cannot tell" - and a renderer that
			 * cannot tell must assume nothing moved, so it kept the dead
...
#include "gl-bridge-host.inc"
		...
		default:
			 ...
			 * gate's gpu leg printed this line 60 times per 60-frame swiss run
			 * in EACH flavour - run-native dispatches through this file too -
			 * and PASSED, because both flavours were shrugged at identically
			 * and their frame lines compared equal.
